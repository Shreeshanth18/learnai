...
function Shell({ children }: { children: ReactNode }) {
  ...
  const secondaryLinks = [{ href: '/settings', label: 'Settings', icon: Settings2 }];
  return <div className="app-surface min-h-[100dvh] overflow-x-hidden text-foreground noise">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col bg-sidebar px-4 py-5 text-sidebar-foreground shadow-[18px_0_50px_hsl(203_35%_17%_/_0.12)] transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="mb-8 flex items-center justify-between px-2">
        <Link href={trainerView ? '/admin' : '/workspace'} onClick={() => setMobileOpen(false)} className="flex items-center gap-3" data-testid="link-brand">
          ...
          <span><span className="block font-display text-lg font-extrabold tracking-tight">learn<span className="text-sidebar-primary">ai</span></span><span className="block font-mono-ui text-[9px] uppercase tracking-[.18em] text-sidebar-foreground/50">{trainerView ? 'trainer console' : 'competency studio'}</span></span>
        </Link>
      ...
      {trainerView && <>
        <div className="mb-3 px-3 font-mono-ui text-[10px] uppercase tracking-[.16em] text-sidebar-foreground/40">Trainer workspace</div>
        <nav className="space-y-1" aria-label="Trainer navigation">
          <Link href="/admin" onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${location === '/admin' ? 'bg-sidebar-accent text-sidebar-foreground' : 'text-sidebar-foreground/65 hover:bg-sidebar-accent hover:text-sidebar-foreground'}`} data-testid="link-nav-admin"><Users size={17} /><span>Learner records</span></Link>
        </nav>
      </>}
      {!trainerView && <>
        <div className="mt-7 px-3 font-mono-ui text-[10px] uppercase tracking-[.16em] text-sidebar-foreground/40">Studio</div>
        <nav className="mt-2 space-y-1">{secondaryLinks.map(({ href, label, icon: Icon }) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${location === href ? 'bg-sidebar-accent text-sidebar-foreground' : 'text-sidebar-foreground/65 hover:bg-sidebar-accent hover:text-sidebar-foreground'}`} data-testid={`link-nav-${label.toLowerCase().replaceAll(' ', '-')}`}><Icon size={17} /><span>{label}</span></Link>)}</nav>
      </>}
      <div className="mt-auto rounded-2xl border border-sidebar-border bg-sidebar-accent/60 p-3">
        ...
        <div className="flex items-center justify-between gap-2 text-[11px] text-sidebar-foreground/55"><span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-sidebar-primary" />{trainerView ? 'Trainer access' : 'Secure workspace'}</span><button onClick={() => { localStorage.removeItem('learnai_trainer_mode'); void signOut({ redirectUrl: basePath || '/' }); }} className="inline-flex items-center gap-1 rounded-lg px-1.5 py-1 text-sidebar-foreground/55 transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground" data-testid="button-sign-out"><LogOut size={12} />Sign out</button></div>
      </div>
    </aside>
    {mobileOpen && <button className="fixed inset-0 z-30 bg-sidebar/40 backdrop-blur-sm lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation" data-testid="button-overlay-menu" />}
     <div className="lg:pl-[260px]">
      <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-border/70 bg-background/75 px-4 shadow-[0_8px_30px_hsl(201_34%_18%_/_0.04)] backdrop-blur-xl sm:px-8">
         ...
         <div className="hidden items-center gap-2 text-xs text-muted-foreground sm:flex"><span className="h-2 w-2 rounded-full bg-primary" />{trainerView ? 'Trainer workspace' : 'Personal workspace'} <span className="text-border">/</span> {trainerView ? 'Learner records' : location === '/workspace' ? 'Overview' : location.slice(1).replace('-', ' ')}</div>
        <div className="ml-auto flex items-center gap-2"><button onClick={toggleTheme} className="rounded-xl p-2.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" aria-label="Toggle theme" data-testid="button-toggle-theme">{dark ? <Sun size={17} /> : <Moon size={17} />}</button><Link href={trainerView ? '/admin' : '/settings'} className="grid h-9 w-9 place-items-center rounded-full bg-primary/15 text-xs font-bold text-primary" data-testid="link-header-profile">{profile.name.split(' ').map((part) => part[0]).join('')}</Link></div>
      </header>
      <main className="mx-auto min-w-0 max-w-[1440px] px-4 pb-24 pt-6 sm:p-8 sm:pb-8">{children}</main>
       {!trainerView && <nav className="safe-area-bottom fixed inset-x-0 bottom-0 z-20 grid grid-cols-4 border-t border-border/80 bg-card/95 px-2 pt-2 shadow-[0_-8px_24px_hsl(201_34%_18%_/_0.06)] backdrop-blur-lg lg:hidden" aria-label="Quick navigation">{navItems.slice(0, 4).map(({ href, label, icon: Icon }) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-[10px] font-bold ${location === href ? 'bg-primary/10 text-primary' : 'text-muted-foreground'}`} data-testid={`link-mobile-nav-${label.toLowerCase().replaceAll(' ', '-')}`}><Icon size={17} strokeWidth={location === href ? 2.5 : 1.8} /><span className="max-w-full truncate">{label === 'Learning path' ? 'Learn' : label === 'Adaptive quiz' ? 'Quiz' : label}</span></Link>)}</nav>}
    </div>
  </div>;
}
...
function AuthRouteLink({ href, children, className = '' }: { href: string; children: ReactNode; className?: string }) {
  ...
  return <a href={href} className={className} onClick={async (event) => { event.preventDefault(); localStorage.removeItem('learnai_trainer_mode'); await signOut(); setLocation(href); }}>{children}</a>;
}

function AuthPage({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  useEffect(() => {
    localStorage.removeItem('learnai_trainer_mode');
  }, []);
...
function TrainerLoginPage() {
  ...
  useEffect(() => {
    localStorage.setItem('learnai_trainer_mode', 'true');
    if (isSignedIn) setLocation('/admin');
  }, [isSignedIn, setLocation]);
  return <div className="surface-grid grid min-h-[100dvh] place-items-center bg-sidebar px-5 py-8 text-sidebar-foreground"><div className="w-full max-w-[520px] animate-enter"><div className="mb-8 text-center"><BrandMark dark /><p className="mt-8 font-mono-ui text-[10px] uppercase tracking-[.2em] text-sidebar-primary">Facilitator console</p><h1 className="mt-3 font-display text-4xl font-extrabold">Trainer sign in.</h1><p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-sidebar-foreground/60">Access learner records, cohort signals, and management tools for trainer operations only.</p></div><div className="rounded-2xl border border-sidebar-border bg-sidebar-accent/80 p-2 shadow-2xl"><div className="rounded-xl border border-sidebar-border bg-sidebar p-4 sm:p-6"><SignIn routing="path" path={`${basePath}/trainer-login`} signUpUrl={`${basePath}/sign-up`} oauthFlow="redirect" forceRedirectUrl={`${basePath}/admin`} /></div></div><div className="mt-6 flex justify-center gap-5 text-sm font-bold"><AuthRouteLink href="/sign-in" className="text-sidebar-primary hover:underline">Candidate sign in</AuthRouteLink><AuthRouteLink href="/sign-up" className="text-sidebar-primary hover:underline">Candidate sign up</AuthRouteLink></div></div></div>;
}

function Router() {
  const { isLoaded, isSignedIn } = useAuth();
  const [location, setLocation] = useLocation();
  const trainerMode = typeof window !== 'undefined' && localStorage.getItem('learnai_trainer_mode') === 'true';
  ...
  useEffect(() => {
    if (!isLoaded) return;
    if (trainerMode && isSignedIn && !location.startsWith('/admin')) {
      setLocation('/admin');
      return;
    }
    if (isSignedIn && (location === '/' || authPath)) {
      setLocation(trainerMode ? '/admin' : '/workspace');
      return;
    }
    if (trainerMode && protectedPath && !location.startsWith('/admin')) {
      setLocation('/admin');
      return;
    }
    if (!isSignedIn && protectedPath) setLocation('/');
  }, [authPath, isLoaded, isSignedIn, location, protectedPath, setLocation, trainerMode]);
  ...
  if (!isSignedIn) return <Switch><Route path="/sign-in/*?" component={SignInPage} /><Route path="/sign-up/*?" component={SignUpPage} /><Route path="/trainer-login/*?" component={TrainerLoginPage} /><Route path="/home" component={PublicHome} /><Route component={SignInPage} /></Switch>;
    ...
    if (trainerMode && isSignedIn && !location.startsWith('/admin')) {
      return <div className="grid min-h-[100dvh] place-items-center bg-sidebar text-sidebar-foreground"><div className="animate-pulse-soft text-center"><BrandMark dark /><p className="mt-5 text-xs uppercase tracking-[.18em] text-sidebar-primary">Redirecting to trainer console</p></div></div>;
    }
